...
do = pd.DataFrame(R, columns = ["fid", "smiles"] + ["promiscuity_fxp{0}_pxf{1}".format(x[0], x[1]) for x in slots])

fid2pid_counts = dict((k, len(v)) for k,v in fid2pid.items())
fid2pid_counts = sorted(fid2pid_counts.items(), key=lambda x: -x[1])

cuts_pxf = [200, 300, 400]
...
dp = pd.DataFrame(R, columns = ["fid", "smiles"] + ["promiscuity_pxf{0}".format(i) for i in range(len(cuts_pxf))])

dp.to_csv(os.path.join(data_dir, "promiscuity_pxf.csv"), index=False)
do.to_csv(os.path.join(data_dir, "promiscuity_pxf_fxp.csv"), index=False)