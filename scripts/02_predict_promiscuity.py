...
ds = pd.read_csv(os.path.join(data_dir, "promiscuity_pxf.csv"))
...
df.to_csv(os.path.join(results_dir, "promiscuity_pxf_predictions_enamine_stock.csv"), index=False)

df = pd.read_csv(os.path.join(data_dir, "enamine_stock.csv"))
ds = pd.read_csv(os.path.join(data_dir, "promiscuity_pxf_fxp.csv"))
headers = list(ds.columns)[2:]
...
df = pd.DataFrame(R, columns = ["catalog_id", "smiles"] + headers)
df.to_csv(os.path.join(results_dir, "promiscuity_pxf_fxp_predictions_enamine_stock.csv"), index=False)